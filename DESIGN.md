...
โครงหน้าเว็บทุกหน้าอยู่ภายใต้โครงร่างเดียวกัน คือ แถบหัวเรื่อง (Brand bar + เมนูระบบเฉพาะจอ md ขึ้นไป) → Breadcrumb → เนื้อหาในคอนเทนเนอร์กว้างสูงสุด 1360px → ส่วนท้ายเว็บ → แถบเมนูล่างแบบแอปโทรศัพท์ (เฉพาะจอเล็ก)
...
### Navigation & Tabs
*   **ลำดับระบบ (ทุกที่ที่แสดงรายการระบบ):** 1 จองห้องประชุม → 2 แจ้งซ่อม → 3 ยืมอุปกรณ์
    ใช้ลำดับเดียวกันทั้งเมนูบน แถบเมนูล่าง การ์ดหน้าแรก และลำดับการปัดเปลี่ยนหน้า
*   **แถบเมนูล่าง (มือถือ, `BottomNav`):** จอเล็ก (< 768px) เมนูระบบย้ายลงล่างเป็น 4 ไอคอน
    (หน้าแรก / จองห้อง / แจ้งซ่อม / ยืมของ) สูง 64px + safe area ของเครื่อง ไอคอนขนาด 20px
    พร้อมป้ายชื่อ `label-sm` ใต้ไอคอน เมนูที่เลือกใส่พิลสีพื้น `secondary-fixed` รอบไอคอน
    และตัวอักษรสี `primary` ตัวหนา — เนื้อหาเว้นที่ท้ายหน้าด้วย `--bottom-nav-height`
*   **แท็บระบบ (header):** เฉพาะจอ md ขึ้นไป — แท็บที่เลือกพื้น `primary-container` ตัวอักษรขาว มุมมน 8px
